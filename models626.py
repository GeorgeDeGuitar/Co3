...
class BoundaryDiscriminator(nn.Module):
    ...
    def __init__(self, input_channels=1, input_size=33):
        ...
        # 导入边界提取器
        from BoundaryDis import BoundaryDetector
        self.true_extractor = BoundaryDetector(input_size)
        
        # 简单的卷积网络，用于处理input与boundary mask相乘后的结果
        # 输入是 input * boundary_mask，所以输入通道数还是input_channels
        
        # 第一层卷积
        self.conv1 = spectral_norm(nn.Conv2d(input_channels, 64, kernel_size=3, stride=2, padding=1))
        self.bn1 = nn.BatchNorm2d(64)
        self.act1 = nn.LeakyReLU(0.2, inplace=True)
    ...
    def forward(self, local_input, mask):
        """
        Args:
            local_input: 输入图像 [batch, channels, H, W]
            mask: 掩码 [batch, 1, H, W]
        Returns:
            boundary_features: 1024维边界特征
        """
        # 检查输入是否包含NaN
        if torch.isnan(local_input).any():
            print("警告: BoundaryDiscriminator输入包含NaN!")
            local_input = torch.nan_to_num(local_input, nan=0.0)
        
        # 使用边界提取器提取边界mask
        with torch.no_grad():  # 边界提取不需要梯度
            boundary_mask = self.true_extractor(mask)  # 提取边界
        
        # 将输入与边界mask相乘，突出边界区域
        boundary_input = local_input * boundary_mask
        
        # 通过卷积网络处理边界输入
        x = self.act1(self.bn1(self.conv1(boundary_input)))
        x = self.act2(self.bn2(self.conv2(x)))
        x = self.act3(self.bn3(self.conv3(x)))
        x = self.act4(self.bn4(self.conv4(x)))
        ...
        # 检查实际特征尺寸与预期尺寸是否匹配
        if x.size(1) != self.feature_size:
            print(f"警告: BoundaryDiscriminator特征尺寸不匹配! 预期 {self.feature_size}，实际 {x.size(1)}")
            # 动态调整线性层
            if not hasattr(self, '_feature_size_adjusted') and not self.training:
                self._feature_size_adjusted = True
                self.linear = nn.Sequential(
        ...
        # 输出1024维特征
        boundary_features = self.linear(x)
        
        # 检查输出是否包含NaN
        if torch.isnan(boundary_features).any():
            print("警告: BoundaryDiscriminator输出包含NaN!")
            boundary_features = torch.nan_to_num(boundary_features, nan=0.0)
        
        return boundary_features


class ContextDiscriminator(nn.Module):
    """修改后的上下文判别器，支持Local、Global、Boundary三者特征融合"""
    def __init__(
        self,
        ...
        self.model_bd = BoundaryDiscriminator(
            input_channels=local_input_channels, input_size=local_input_size
        )
        ...
        self.flatten_bd = nn.Flatten()

        # 特征融合层 - 现在需要处理3个1024维的特征
        self.fusion = nn.Sequential(
            spectral_norm(nn.Linear(1024 + 1024 + 1024, 1024)),  # 3 * 1024 -> 1024
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout(0.6)
        )
        ...
        self.classifier = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout(0.5),
            spectral_norm(nn.Linear(512, 1))  # 输出logits
        )
    ...
    def forward(self, x_ld, x_lm, x_gd, x_gm):
        ...
        """
        # 通过三个判别器提取特征
        x_ld_features = self.model_ld(x_ld, x_lm)  # 1024维局部特征
        x_gd_features = self.model_gd(x_gd, x_gm)  # 1024维全局特征
        x_bd_features = self.model_bd(x_ld, x_lm)  # 1024维边界特征

        # 确保形状正确（防止意外的高维张量）
        if len(x_ld_features.shape) > 2:
            x_ld_features = self.flatten_ld(x_ld_features)
        if len(x_gd_features.shape) > 2:
            x_gd_features = self.flatten_gd(x_gd_features)
        if len(x_bd_features.shape) > 2:
            x_bd_features = self.flatten_bd(x_bd_features)

        # 拼接三个特征 [batch, 3072]
        combined = torch.cat([x_ld_features, x_gd_features, x_bd_features], dim=1)
        ...
        # 返回判别结果和各个特征（用于调试或其他用途）
        return logits, x_ld_features, x_gd_features, x_bd_features
