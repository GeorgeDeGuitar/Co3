...
def save_checkpoint(
    ...
    try:
        ...
        checkpoint = {
            ...
            "model_cd_state_dict": (
                model_cd.state_dict() if model_cd is not None else None
            ),
            "model_bqd_state_dict": (
                model_bqd.state_dict() if model_bqd is not None else None
            ),
...
def load_checkpoint(
    ...
    try:
        ...
        if model_bqd is not None and checkpoint.get("model_bqd_state_dict") is not None:
            print("加载边界质量判别器状态...")
            model_bqd.load_state_dict(checkpoint["model_bqd_state_dict"])

        # 加载优化器状态
        if (
            "opt_cn_state_dict" in checkpoint
            and checkpoint["opt_cn_state_dict"] is not None
        ):
            print("加载生成器优化器状态...")
            opt_cn.load_state_dict(checkpoint["opt_cn_state_dict"])

        if opt_cd is not None and checkpoint.get("opt_cd_state_dict") is not None:
            print("加载判别器优化器状态...")
            opt_cd.load_state_dict(checkpoint["opt_cd_state_dict"])

        if opt_bqd is not None and checkpoint.get("opt_bqd_state_dict") is not None:
            print("加载边界质量判别器优化器状态...")
        ...
        return step, phase, best_val_loss, best_acc, cn_lr, cd_lr, bqd_lr
        ...
        if os.path.exists(backup_path):
            ...
            try:
                return load_checkpoint(
                    backup_path, model_cn, model_cd, opt_cn, opt_cd, device
                )
        ...
        if os.path.exists(latest_path) and latest_path != checkpoint_path:
            ...
            try:
                return load_checkpoint(
                    latest_path, model_cn, model_cd, opt_cn, opt_cd, device
                )
...
def train(dir, envi, cuda, batch, test=False, resume_from=None, Phase=1):
    try:
                        ...
                        try:
                            ...
                            if True:
                                if outputs is not None:
                                    ...
                                    adversarial_loss = bqd_loss_mask_adversarial/ accumulation_steps  # 降低权重避免过强
                                    ...
                                    print("Warning: outputs is None, skipping adversarial loss")
                            ...
                            pbar.set_description(
                                f"Phase 1 | train loss: {loss.item():.5f}"
                            )
                            pbar.update(1)
        ...
        def feature_contrastive_loss(
            real_local_feat, real_global_feat, fake_local_feat, fake_global_feat
        ):
            """特征对比损失 - 鼓励真假样本特征分离"""
                            ...
                            if step % snaperiod_2 == 0:
                                try:
                                    ...
                                    save_checkpoint(
                                        ...
                                        result_dir,
                                        best_acc=best_acc,
                                        cd_lr=opt_cd.param_groups[0]["lr"],
                                    )
                            ...
                            if viz is not None:
                                try:
                                    ...
                                    if "phase3_gen" in loss_windows:
                                        viz.line(
                                            Y=torch.tensor(
                                                [loss_cn.item() * accumulation_steps]
                                            ),
                                            X=torch.tensor([step]),
                            ...
                            if step % snaperiod_3 == 0:
                                try:
                                    val_results = validateAll(
                                        model_cn, model_cd, val_subset_loader, device
                                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--resume",
        type=str,
        default=r"E:\KingCrimson Dataset\Simulate\data0\results20\latest_checkpoint.pth",
        help="resume from checkpoint path",
    )
    parser.add_argument(
        "--dir", type=str, default="results20", help="directory to save results"
    )
    parser.add_argument(
        "--envi", type=str, default="DEM20", help="visdom environment name"
    )
