...
class DemDataset(Dataset):
    ...
    def _analyze_data_quality(self):
        ...
        sample_indices = np.random.choice(len(self.original_data_groups), sample_size, replace=False)
    ...
    def _get_original_item(self, idx):
        ...
        for retry in range(max_retries):
            try:
                ...
                # 获取数据组键
                key = next(itertools.islice(self.original_data_groups.keys(), actual_idx, actual_idx + 1))
                group_data = self.original_data_groups[key]
                json_files = group_data["json_files"]
                meta = group_data["meta"]
                ...
                if retry == max_retries - 1:
                    print(f"Failed to load original data after {max_retries} retries for index {idx}: {e}")
                continue
    ...
    def _get_synthetic_item(self, idx):
        ...
        try:
            ...
            # 获取原始数据的所有组件
            key = next(itertools.islice(self.original_data_groups.keys(), original_idx, original_idx + 1))
            group_data = self.original_data_groups[key]
            json_files = group_data["json_files"]
            meta = group_data["meta"]
    ...
    def visualize_synthetic_mask(self, num_samples=4, save_dir=None):
        ...
        for i in range(num_samples):
            ...
            (input_data, input_mask, target_data, 
             global_array, global_mask, global_target, metadata) = self[synthetic_idx]
...
if __name__ == "__main__":
    ...
    if dataset.enable_synthetic_masks and len(dataset) > dataset.original_length:
        print("🎨 生成数据增强可视化...")
        dataset.visualize_synthetic_mask(num_samples=4, save_dir="./mask_visualization_results")
    ...
    print(f"   - 总数据量: {len(dataset)} (增长 {(len(dataset)/dataset.original_length-1)*100:.0f}%)")
