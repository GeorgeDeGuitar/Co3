...
def train(dir, envi, cuda, batch, test=False, resume_from=None, Phase=1):
    try:
                ...
                loss_windows = {
                    ...
                    "phase3_gen": viz.line(
                        ...
                        opts=dict(
                            ...
                            legend=["gen loss", "bqd loss", "boundary loss"],  # 移到opts内部
                            linecolor=np.array([[255, 0, 0], [0, 255, 0], [0, 0, 255]]),
                        ),
                    ),
        ...
        def feature_contrastive_loss(
            real_local_feat, real_global_feat, real_boundary_feat, fake_local_feat, fake_global_feat, fake_boundary_feat
        ):
            """特征对比损失 - 鼓励真假样本特征分离"""
                            ...
                            if step % snaperiod_3 == 0:
                                try:
                                    val_results = validateAll(
                                        model_cn, model_cd, model_bqd, val_subset_loader, device
                                    )
