    ...
    def window_based_loss(self, output, target, mask=None):
        ...
        for c in range(channels):
            for b in range(batch_size):
                ...
                # 使用卷积操作高效计算窗口内的和
                padded_diff = F.pad(
                    pixel_diff_squared, (pad_size, pad_size, pad_size, pad_size)
                )
...
def edge_continuity_loss(completed, global_target, pos, edge_width, completed_mask):
    ...
    for b in range(batch_size):
        ...
        # 提取全局目标和补全区域的扩展区域
        global_region = global_target[
            b : b + 1, :, extended_xmin:extended_xmax, extended_ymin:extended_ymax
        ]
        completed_region = completed[
            b : b + 1, :, extended_xmin:extended_xmax, extended_ymin:extended_ymax
        ]
        comtarget_region = completed_mask[
            b : b + 1, :, extended_xmin:extended_xmax, extended_ymin:extended_ymax
        ]  # 是否应该考虑？
        ...
        # 生成坐标网格
        y_coords = (
            torch.arange(ext_h, device=completed.device).view(-1, 1).repeat(1, ext_w)
        )
        x_coords = (
            torch.arange(ext_w, device=completed.device).view(1, -1).repeat(ext_h, 1)
        )
        ...
        # 扩展权重掩码的维度
        weight_mask = weight_mask.view(1, 1, ext_h, ext_w).expand(
            1, channels, ext_h, ext_w
        )

        # 1. 计算像素差异 (0阶导数误差)
        pixel_diff = (
            torch.abs(completed_region - global_region) * weight_mask * comtarget_region
        )
        ...
        grad_diff_h = (
            torch.abs(out_grad_h - global_grad_h) * weight_mask_h * comtarget_region_h
        )
        grad_diff_w = (
            torch.abs(out_grad_w - global_grad_w) * weight_mask_w * comtarget_region_w
        )

        one_loss += (grad_diff_h.sum() + grad_diff_w.sum()) / (
            (weight_mask_h * comtarget_region_h).sum()
            + (weight_mask_w * comtarget_region_w).sum()
            + 1e-8
        )
        ...
        # 并行计算所有二阶导数差异
        grad2_diff_h = (
            torch.abs(out_grad2_h - global_grad2_h)
            * weight_mask_h2
            * comtarget_region_h2
        )
        grad2_diff_w = (
            torch.abs(out_grad2_w - global_grad2_w)
            * weight_mask_w2
            * comtarget_region_w2
        )
        grad2_diff_diag = (
            torch.abs(out_grad2_diag - global_grad2_diag)
            * weight_mask_diag
            * comtarget_region_diag
        )

        two_loss += (
            grad2_diff_h.sum() + grad2_diff_w.sum() + grad2_diff_diag.sum()
        ) / (
            (weight_mask_h2 * comtarget_region_h2).sum()
            + (weight_mask_w2 * comtarget_region_w2).sum()
            + (weight_mask_diag * comtarget_region_diag).sum()
            + 1e-8
        )
...
class EnhancedGradientConsistencyLoss(nn.Module):
    ...
    def forward(self, output, target, mask):
        ...
        magnitude_loss = F.l1_loss(
            output_mag * boundary_weight, target_mag * boundary_weight
        )
...
def total_variation_loss(pred, mask=None, dilation_kernel_size=3, visualize=False):
    ...
    if mask is not None:
        ...
        # 使用卷积核检测边界
        kernel = torch.ones(
            1, 1, 3, 3, device=mask_float.device, dtype=mask_float.dtype
        )  # 3x3 卷积核
        mask_padded = F.conv2d(
            mask_float, kernel, padding=1
        )  # 卷积后形状仍为 [B, C, H, W]
        # 边界定义为 mask 值变化的区域
        boundary_mask = (mask_padded > 0) & (
            mask_padded < 9
        )  # 9 是 3x3 卷积核的最大值（全 1 时）
        ...
        if dilation_kernel_size > 1:
            kernel = torch.ones(
                ...
                dtype=mask_float.dtype,
            )
            boundary_mask = (
                F.conv2d(boundary_mask, kernel, padding=dilation_kernel_size // 2) > 0
            )
...
def gradient_similarity_loss(
    pred, gt, mask=None, dilation_kernel_size=3, visualize=False
):
    """
...
def ssim_loss(pred, gt, mask=None, window_size=5, sigma=1.5, size_average=True):
    ...
    sigma_pred_sq = (
        F.conv2d(pred * pred, window, padding=window_size // 2, groups=1) - mu_pred_sq
    )
    sigma_gt_sq = (
        F.conv2d(gt * gt, window, padding=window_size // 2, groups=1) - mu_gt_sq
    )
    sigma_pred_gt = (
        F.conv2d(pred * gt, window, padding=window_size // 2, groups=1) - mu_pred_gt
    )
...
def input_smoothness_loss(output, mask, sigma=0.5):
    ...
    laplacian_kernel = torch.tensor(
        [[0, 1, 0], [1, -4, 1], [0, 1, 0]], dtype=output.dtype, device=output.device
    )
...
def completion_network_loss(
    ...
    unsqueezed_lt = local_target.unsqueeze(1)  # [B, H, W]
    # print(f"local_target shape: {sunqueezed_lt.shape}, output shape: {output.shape}, mask shape: {mask.shape}")
    null_loss = mse_loss(
        unsqueezed_lt * (1 - mask), output * (1 - mask)
    )  # target 是目标补全图像
    """valid_loss = mse_loss(
    ...
    consistency_loss = (
        spatial_consistency.gradient_based_loss(output, local_target, mask) * 10
    )  # 空间一致性损失，可选择输入mask
    ...
    insm_loss = (
        input_smoothness_loss(output, mask, sigma=0.5) * 0.02
    )  # 输入平滑性损失，鼓励生成输出在边界区域的平滑性
